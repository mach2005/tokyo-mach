...
function Format-Song {
    ...
    foreach ($line in $lines) {
        ...
        $headerHtml = ""
        $audioHtml = ""
        # Map player numbers to audio files (Relative to /hp/ folder)
        if ($m.Success) {
            $num = $m.Groups[1].Value
            $name = $m.Groups[2].Value
            $headerHtml = "<span class='song-num'>$num</span><span class='song-name'>$name</span>"
            switch ($num) {
            ...
            $headerHtml = "<span class='generic-badge'>$titleRaw</span>"
        }
...
[System.IO.File]::WriteAllText("$hp\songs.html", $finalHtml, [System.Text.Encoding]::UTF8)
Write-Output "Done: songs.html created in $hp"
